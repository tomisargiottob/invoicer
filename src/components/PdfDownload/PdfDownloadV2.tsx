...
function PDFDownloadV2({invoice, cuit}: {invoice: Invoice, cuit: CuitAccount}) {
    ...
    const total = invoice.items.reduce((amount: {iva: {[k: string]: number}, grossAmount: number, netAmount: number}, item) => {
      if (!amount.iva[item.iva!]) amount.iva[item.iva!] = 0
      amount.iva[item.iva!] += item.units! * item.unitValue! * (item.iva! / 100)
          ...
          return (
              ...
              <div className="borderDiv periodContainer">
                <div className="label">Periodo Facturado Desde: 
                  {perdesDate && <span className="data">{convertDateToDDMMAAAASeparated2(perdesDate)}</span>}
                </div>
                <div className="label">Hasta: 
                  {perhasDate && <span className="data">{convertDateToDDMMAAAASeparated2(perhasDate.toISOString())}</span>}
                </div>
              </div>
              ...
              <div className="borderDiv invoiceTotal vat-description">
                <div className="vat-types">
                  {['A', 'NOTA_CREDITO_A'].includes(invoice.invoiceType) ? (<>
                      <div className="label">Subtotal: </div><div>$ {total.grossAmount}</div>
